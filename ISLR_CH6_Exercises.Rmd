...
# First create predictor matrix and response vector
predictorX <- model.matrix(Direction~., weekly.df)[, -1]
responseY <- weekly.df$Direction

# next use PCA to find principal components:
(pca <- princomp(predictorX, cor=T)) # PCA using correlation matrix

weekly.pca <- as_tibble(pca$scores[,]*-1) %>% 
  add_column(Direction=responseY)

(pca.cols <- colnames(weekly.pca[ , !(names(weekly.pca) %in% c("Direction"))]))

result <- NULL

for (pc.col.idx in 1:length(pca.cols)){
  pca.chosen.cols <- pca.cols[1:pc.col.idx]
  result <- bind_rows(result,c(no.of.pcas = pc.col.idx)) 
}
result
```
